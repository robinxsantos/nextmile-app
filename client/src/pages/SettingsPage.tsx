...
export default function SettingsPage() {
  ...
  const inputClass = 'w-full min-h-[44px] rounded-[14px] border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 text-slate-900 dark:text-slate-100 px-3.5 text-sm focus:border-blue-400 dark:focus:border-blue-500 focus:ring-2 focus:ring-blue-500/10 outline-none transition-colors';
    ...
    <div>
      <div className="glass-card rounded-[28px] border border-slate-200/80 dark:border-slate-700/90 shadow-lg p-5 mb-3.5">
        <div className="flex flex-col md:flex-row justify-between md:items-end gap-3">
          <div>
            <h1 className="text-[1.45rem] font-bold tracking-tight">Settings</h1>
            <p className="text-sm text-slate-500 mt-1">Update your profile and change your password.</p>
          </div>
          <div className="text-right">
            <div className="text-[0.72rem] font-bold tracking-wider uppercase text-slate-500">Account</div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-3.5 items-stretch">
        {/* Profile Section */}
        <div className="glass-card rounded-[22px] border border-slate-200/90 dark:border-slate-700/90 shadow-sm p-5 flex flex-col h-full">
          ...
          <form onSubmit={handleUpdateProfile} className="flex flex-col gap-4 flex-1">
            <div>
              ...
              <p className="text-[0.65rem] text-slate-400 mt-1">Username cannot be changed</p>
            </div>
            <div>
              <label className="text-xs font-semibold text-slate-600 dark:text-slate-300 mb-1.5 block">Display Name</label>
            ...
            <div>
              ...
              <input type="text" value={user?.role === 'admin' ? 'Admin' : 'Driver'} disabled className={inputClass + ' opacity-50 cursor-not-allowed'} />
            </div>
        ...
        <div className="glass-card rounded-[22px] border border-slate-200/90 dark:border-slate-700/90 shadow-sm p-5 flex flex-col h-full">
          <div className="flex items-center gap-2.5 mb-4">
            ...
            <div>
              <h2 className="text-base font-bold tracking-tight">Change Password</h2>
          ...
          <form onSubmit={handleChangePassword} className="flex flex-col gap-4 flex-1">
            <div>
              <label className="text-xs font-semibold text-slate-600 dark:text-slate-300 mb-1.5 block">Current Password</label>
            ...
            <div>
              <label className="text-xs font-semibold text-slate-600 dark:text-slate-300 mb-1.5 block">New Password</label>
            ...
            <div>
              <label className="text-xs font-semibold text-slate-600 dark:text-slate-300 mb-1.5 block">Confirm New Password</label>
